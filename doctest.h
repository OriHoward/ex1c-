...
// - relational operators as free functions - taking const char* as one of the params
        ...
        namespace Color {
            ...
            DOCTEST_INTERFACE std::ostream & operator << (std::ostream & s, Color::Enum
            code);
        } // namespace Color
        ...
        struct DOCTEST_INTERFACE SubcaseSignature
        {
            ...
            bool operator<(const SubcaseSignature
            &other) const;
        };
        ...
        struct DOCTEST_INTERFACE IContextScope
        {
            ...
            virtual
            void stringify(std::ostream *) const = 0;
        };
        ...
        namespace detail {
            ...
            String
            rawMemoryToString(
            const DOCTEST_REF_WRAP(T)object) {
        ...
        class DOCTEST_INTERFACE Approx
        {
            ...
            // clang-format off
            DOCTEST_INTERFACE friend
            bool operator==(double lhs,
            const Approx
            &rhs);
            DOCTEST_INTERFACE friend
            bool operator==(const Approx
            &lhs,
            double rhs);
            DOCTEST_INTERFACE friend
            bool operator!=(double lhs,
            ...
            double rhs);
            DOCTEST_INTERFACE friend
            bool operator<(double lhs, const Approx
            &rhs);
            DOCTEST_INTERFACE friend
            bool operator<(const Approx
            &lhs,
...
#define DOCTEST_CMP_LE(l, r) le(l, r)
            ...
            struct DOCTEST_INTERFACE ExpressionDecomposer {
                ...
                ExpressionDecomposer(assertType::Enum
                at);
                ...
                Expression_lhs< const DOCTEST_REF_WRAP(L)> operator<<(const DOCTEST_REF_WRAP(L) operand) {
                    ...
                    const DOCTEST_REF_WRAP(L) > (operand, m_at);
                }
            };
            ...
            DOCTEST_INTERFACE int regTest(const TestCase
            &tc);
            DOCTEST_INTERFACE int setTestSuite(const TestSuite
            &ts);
...
#define DOCTEST_BINARY_RELATIONAL_OP(n, op) \
    template <class L, class R> struct RelationalComparator<n, L, R> { bool operator()(const DOCTEST_REF_WRAP(L) lhs, const DOCTEST_REF_WRAP(R) rhs) const { return op(lhs, rhs); } };
            ...
            DOCTEST_INTERFACE void failed_out_of_a_testing_context(const AssertData
            &ad);
...
#define DOCTEST_ASSERT_IN_TESTS(decomp)                                                            \
    ...
    throwException()
            ...
            template<
            int comparison, typename
            L, typename
            R >
            DOCTEST_NOINLINE
            void binary_assert(assertType::Enum at, const char *file, int line,
            const char *expr,
            const DOCTEST_REF_WRAP(L)lhs,
            const DOCTEST_REF_WRAP(R)rhs) {
            ...
            L >
            DOCTEST_NOINLINE
            void unary_assert(assertType::Enum at, const char *file, int line,
            const char *expr,
            ...
            template < typename
            T >
            void toStream(std::ostream *s, const T
            &value) {
            ...
            class DOCTEST_INTERFACE ContextScopeBase :
            public IContextScope{
                    protected:
                    ContextScopeBase();
            ...
            template < typename
            L >
            ContextScope < L > MakeContextScope(
            const L
            &lambda) {
...
namespace doctest{
        ...
        struct DOCTEST_INTERFACE IReporter
        {
            ...
            virtual
            void report_query(const QueryData
            &) = 0;

            // called when the whole test run starts
            virtual
            void test_run_start() = 0;
            // called when the whole test run ends (caching a pointer to the input doesn't make sense here)
            virtual
            void test_run_end(const TestRunStats
            &) = 0;

            // called when a test case is started (safe to cache a pointer to the input)
            virtual
            void test_case_start(const TestCaseData
            &) = 0;
            // called when a test case is reentered because of unfinished subcases (safe to cache a pointer to the input)
            virtual
            void test_case_reenter(const TestCaseData
            &) = 0;
            // called when a test case has ended
            virtual
            void test_case_end(const CurrentTestCaseStats
            &) = 0;
            ...
            virtual
            void subcase_start(const SubcaseSignature
            &) = 0;
            ...
            virtual
            void log_assert(const AssertData
            &) = 0;
            // called for each message (don't cache pointers to the input)
            virtual
            void log_message(const MessageData
            &) = 0;

            // called when a test case is skipped either because it doesn't pass the filters, has a skip decorator
            // or isn't in the execution range (between first and last) (safe to cache a pointer to the input)
            virtual
            void test_case_skipped(const TestCaseData
            &) = 0;
